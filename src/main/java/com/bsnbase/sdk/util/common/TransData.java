...
public class TransData {
    ...
    public static String getTransdata(TransactionRequest request, TransactionUser user) throws IOException, NoSuchAlgorithmException {
        ProposalPackage.Proposal proposal = proposal(request,user);
        byte[] proposalBytes = convertToBytes(proposal);

        byte[] signBytes ;
        try {
            signBytes = SignUtil.sign(user.getPrivateKey(),proposalBytes);
